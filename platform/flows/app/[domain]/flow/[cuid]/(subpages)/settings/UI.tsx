...
export const FlowSettings: React.FC = () => {
  ...
  const [elements, setElements] = useState<DatabaseElement[]>(databaseElements);
  const [submittedValues, setSubmittedValues] =
    useState<FlowProviderFormValues | null>(null);

  const handleElementsChange = (newElements: DatabaseElement[]) => {
    startTransition(() => {
      setElements(newElements);
      console.log('Auto-saving changes', newElements);
    });
  };
