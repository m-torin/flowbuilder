...
import NodeRedFlowConverter from './ConvertNrRf';
...
export const NewFlowForm: React.FC<NewFlowFormProps> = ({ params }) => {
  ...
  const fetchInstanceId = useCallback(async () => {
    try {
      const id = (await getInstanceBySubdomainAction(subdomain)) as string;
      setInstanceId(id);
      form.setFieldValue('instanceId', id);
    } catch (error) {
      console.error('Failed to fetch instance ID:', error);
      form.setErrors({ instanceId: 'Failed to fetch instance ID' });
    }
  }, [subdomain, form]);
  ...
  const handleSubmit = async (values: FlowFormValues) => {
    ...
    try {
      // Integrate subdomain into values if necessary
