...
export const getNode = async (
  ...
  try {
    ...
    const { flow, ...nodeWithoutFlow } = node;
    ...
    console.error(`Failed to fetch node with ID ${cuid}:`, error);
    return null;
  }
};
...
export const readNodeAction = async (
  ...
  try {
    ...
    console.error('Error in readNodeAction:', error);
    return null;
  }
};
...
export const getNodeByRfId = async (
  ...
  try {
    ...
    console.error(`Failed to fetch node with rfId ${rfId}:`, error);
    return null;
  }
};
...
export const upsertNode = async (
  ...
  // console.log(
  //   'upsertNode incoming',
  //   JSON.stringify({ rfNode, flowId }, null, 2),
  //   '\n',
  // );
  ...
  const { nodeName, webhookType, arn, infrastructureId, ...restData } =
    rfNode.data || {};
  ...
  // console.log('upsertNode metadata', JSON.stringify({ metadata }, null, 2), '\n');
  // console.log(
  //   'upsertNode formFields',
  ...
  try {
    ...
    console.error('Failed to upsert a node:', error);
    return null;
  }
};
