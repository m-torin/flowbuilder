...
const handleSourceEvent = (
  ...
  console.log('Processing AWS EventBridge source event:', { input });
...
const handleDestinationEvent = async (
  ...
  console.log('Processing AWS EventBridge destination event:', {
    input,
    config,
  });
...
const handleEnrichmentEvent = async (
  ...
  console.log('Processing AWS EventBridge enrichment event:', {
    input,
    config,
  });
  ...
  if (config.validation?.rules) {
    ...
    for (const rule of config.validation.rules) {
      try {
        ...
        console.log(`Validating rule: ${rule.field} - ${rule.rule}`);
      } catch (error) {
...
export const computeFunction = async (
  ...
  try {
    ...
    console.error('AWS EventBridge compute error:', error);
